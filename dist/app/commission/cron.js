    ...
    const dailyLimit =
      (_a =
        settings === null || settings === void 0
          ? void 0
          : settings.teamManagementDailyLimit) !== null && _a !== void 0
        ? _a
        : 5000;
    const wallets = yield model_1.Wallet.find({
      pendingTeamManagementCommissionOfSideA: { $gt: 0 },
      pendingTeamManagementCommissionOfSideB: { $gt: 0 },
    });
    let processed = 0;
    for (const wallet of wallets) {
      const minSide = Math.min(
        wallet.pendingTeamManagementCommissionOfSideA,
        wallet.pendingTeamManagementCommissionOfSideB
      );
...
const runManagerialCommissionCron = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    const wallets = yield model_1.Wallet.find({
      pendingManagerialCommissionBalance: { $gt: 0 },
    });
    let processed = 0;
    for (const wallet of wallets) {
      ...
      yield model_1.TransactionLog.create({
        ...
        amount,
        balanceAfter: wallet.balance,
        note: "Team management commission transfer",
      });
      processed++;
    }
    return processed;
